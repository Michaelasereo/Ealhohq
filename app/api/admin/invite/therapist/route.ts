...
export async function POST(req: Request) {
  try {
    ...
    const specializations = Array.isArray(body.specializations)
      ? body.specializations.filter((s) => typeof s === "string" && s.trim())
      : [];
    ...
    try {
      ...
      await prisma.therapyTherapist.create({
        data: {
          profileId: uid,
          ...
          qualifications,
          status: "approved",
          sessionRate: new Prisma.Decimal(sessionRateNgn),
          sessionDuration,
        },
      });
    } catch (dbErr) {
      console.error("Invite therapist DB rollback:", dbErr);
    ...
    const html = `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px;">
          ...
          <p style="color: #555; margin-bottom: 24px; line-height: 1.6;">
            You&apos;ve been invited to join Ealho Therapy as a therapist.
    ...
    const sent = await sendTransactionalEmail({
      to: email,
      subject: "You've been invited to join Ealho Therapy",
      html,
    });
    if (!sent.success) {
    ...
    if (phone) {
      ...
      }).catch((err) => console.error("Invite therapist WhatsApp:", err));
    }
